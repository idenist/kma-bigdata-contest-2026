...
class Timer:
    ...
    def __exit__(self, exc_type, exc, tb):
        ...
        if exc_type is None:
            log(f"{self.name} done. elapsed={elapsed:.1f}s")
        else:
...
def parse_args() -> Config:
    ...
    parser.add_argument("--master-grid", required=True, help="processed/master_grid.parquet")
    parser.add_argument("--grid-date-master", required=True, help="processed/grid_date_master")
    parser.add_argument("--fire-history", required=True, help="산불발생이력.csv")
    ...
    parser.add_argument("--solver", choices=["lbfgs", "liblinear", "saga"], default="lbfgs")
...
def main() -> None:
    ...
    with Timer("[4/6] train logistic regression"):
        ...
        try:
            log(f"n_iter_={model.named_steps['model'].n_iter_}")
        except Exception:
            pass
