...
def path_summary(path: Path) -> str:
    if not path.exists():
        return "missing"
    if path.is_file():
        return f"file size={format_bytes(path.stat().st_size)}"

    parquet_files = list(path.rglob("*.parquet"))
    csv_files = list(path.rglob("*.csv"))
    all_files = [child for child in path.rglob("*") if child.is_file()]
    total_size = sum(child.stat().st_size for child in all_files)
    parts = [f"dir files={len(all_files):,}", f"size={format_bytes(total_size)}"]
    if parquet_files:
        parts.append(f"parquet={len(parquet_files):,}")
    if csv_files:
        parts.append(f"csv={len(csv_files):,}")
    return " ".join(parts)


def print_path_status(label: str, path: Path) -> None:
    print(f"[STATUS] {label}: {path} -> {path_summary(path)}", flush=True)


def print_pipeline_config(args: argparse.Namespace, root: Path, source_grid_date: Path, stage_root: Path, final_root: Path) -> None:
    print("\n" + "=" * 80, flush=True)
    print(f"[PIPELINE START] {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    print(f"[CONFIG] root={root}", flush=True)
    print(f"[CONFIG] mode={args.mode} chunk_by={args.chunk_by} months={args.months or 'auto/all'}", flush=True)
    print(f"[CONFIG] start_date={args.start_date or '-'} end_date={args.end_date or '-'}", flush=True)
    print(f"[CONFIG] threads={args.threads} memory_limit={args.memory_limit}", flush=True)
    print(f"[CONFIG] source_grid_date={source_grid_date} -> {path_summary(source_grid_date)}", flush=True)
    print(f"[CONFIG] stage_root={stage_root}", flush=True)
    print(f"[CONFIG] final_root={final_root}", flush=True)
    print("=" * 80, flush=True)


def print_post_step_status(script_name: str, weather_parquet: Path, forest_parquet: Path, terrain_parquet: Path, power_parquet: Path, index_parquet: Path, final_parquet: Path, root: Path) -> None:
    """Print lightweight file-system checks after a step finishes.

    This intentionally avoids reading huge Parquet files. It only checks whether expected
    outputs exist, how many files were written, and the approximate disk size.
    """
    targets_by_script = {
        "01_weather_features.py": [("weather", weather_parquet)],
        "02_forest_features.py": [("forest", forest_parquet)],
        "03_terrain_ywi_features.py": [("terrain_ywi", terrain_parquet)],
        "04_power_access_pei_features.py": [("power_access", power_parquet)],
        "05_index_features.py": [("pffdri_index", index_parquet)],
        "06_fire_target.py": [("target", root / "output" / "target")],
        "09_build_final_dataset.py": [("final", final_parquet)],
    }
    for label, path in targets_by_script.get(script_name, []):
        print_path_status(label, path)


def progress_line(done: int, total: int, started_at: float) -> str:
    elapsed = time.perf_counter() - started_at
    if done <= 0:
        return f"done=0/{total} elapsed={format_seconds(elapsed)} eta=-"
    avg = elapsed / done
    remaining = max(total - done, 0) * avg
    return f"done={done}/{total} elapsed={format_seconds(elapsed)} eta≈{format_seconds(remaining)}"
...
def run_command(cmd: list[str], label: str) -> float:
    print("\n" + "=" * 80, flush=True)
    print(f"[START] {label} at={datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    ...
    try:
        ...
        elapsed = time.perf_counter() - started_at
        print(f"[FAILED] {label} elapsed={format_seconds(elapsed)} returncode={exc.returncode}", flush=True)
        raise
    elapsed = time.perf_counter() - started_at
    print(f"[DONE] {label} elapsed={format_seconds(elapsed)} at={datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    return elapsed
...
def build_command(
    ...
    if script_name == "09_build_final_dataset.py":
...
def main() -> None:
    ...
    print_pipeline_config(args, root, source_grid_date, stage_root, final_root)
    ...
    if args.mode == "parquet" and args.chunk_by != "none":
        ...
        for index, script_name in enumerate(static_scripts, 1):
            ...
            print_post_step_status(script_name, weather_parquet, forest_parquet, terrain_parquet, power_parquet, index_parquet, final_parquet, root)

        chunks = build_chunks(args, source_grid_date)
        print(f"[PLAN] chunks={len(chunks):,} labels={[label for label, _ in chunks[:5]]}{' ...' if len(chunks) > 5 else ''}", flush=True)
        chunk_scripts = ["01_weather_features.py", "03_terrain_ywi_features.py", "05_index_features.py", "09_build_final_dataset.py"]
        print(f"[PLAN] chunk_scripts={chunk_scripts}", flush=True)
        ...
        for chunk_no, (chunk_label, chunk_months) in enumerate(chunks, 1):
            ...
            for script_name in chunk_scripts:
                ...
                print(f"[PROGRESS] before {progress_line(task_no - 1, total_tasks, pipeline_started_at)}", flush=True)
                run_command(cmd, f"task {task_no}/{total_tasks} chunk={chunk_label} {script_name}")
                print_post_step_status(script_name, weather_parquet, forest_parquet, terrain_parquet, power_parquet, index_parquet, final_parquet, root)
                print(f"[PROGRESS] after  {progress_line(task_no, total_tasks, pipeline_started_at)}", flush=True)
    ...
    for script_name in script_order:
        ...
        print(f"[PROGRESS] before {progress_line(script_no - 1, total_scripts, pipeline_started_at)}", flush=True)
        run_command(cmd, f"script {script_no}/{total_scripts} {script_name}")
        print_post_step_status(script_name, weather_parquet, forest_parquet, terrain_parquet, power_parquet, index_parquet, final_parquet, root)
        print(f"[PROGRESS] after  {progress_line(script_no, total_scripts, pipeline_started_at)}", flush=True)
